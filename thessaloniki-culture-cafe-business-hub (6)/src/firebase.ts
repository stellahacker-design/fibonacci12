...
export interface HubDataBundle {
  ...
  aiCommentary?: string;
  sitePassword?: string;
  unifiedYearsFinancials?: any;
  unifiedDailyActuals?: any;
  unifiedManualRevenues?: any;
  updatedAt?: string;
  updatedBy?: string;
}
...
export async function saveHubData(hubId: string, dataBundle: HubDataBundle): Promise<void> {
  ...
  const payload = {
    ...
    updatedBy: dataBundle.updatedBy || "user_session"
  };
  ...
  const timeoutPromise = new Promise<void>((_, reject) => {
    setTimeout(() => {
      const errorObj = new Error("FirebaseError: [code=resource-exhausted]: Option timed out to prevent stuck thread. Possibly Quota Exceeded.");
      (errorObj as any).code = "resource-exhausted";
      reject(errorObj);
    }, 4000);
  });
...
export async function fetchHubData(hubId: string): Promise<HubDataBundle | null> {
  ...
  const timeoutPromise = new Promise<any>((_, reject) => {
    setTimeout(() => {
      const errorObj = new Error("FirebaseError: [code=resource-exhausted]: Option timed out. Possibly Quota Exceeded.");
      (errorObj as any).code = "resource-exhausted";
      reject(errorObj);
    }, 4000);
  });
