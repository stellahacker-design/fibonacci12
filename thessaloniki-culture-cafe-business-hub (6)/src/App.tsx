...
export default function App() {
  ...
  const [financialsTrigger, setFinancialsTrigger] = useState<number>(0);

  useEffect(() => {
    const handleUpdate = () => {
      setFinancialsTrigger(prev => prev + 1);
    };
    window.addEventListener("financials-updated", handleUpdate);
    return () => {
      window.removeEventListener("financials-updated", handleUpdate);
    };
  }, []);
  ...
  const activeZoneCalculations = useMemo(() => {
    return operatingZones.map((section: any) => {
      ...
      const calculatedHourlySales = Math.round(calculatedDailySales / section.duration);
      
      const taQty = Math.round((calculatedDailySales * section.takeAwayPercent) / 100);
      const sitQty = Math.round((calculatedDailySales * section.sitInPercent) / 100);
  ...
  const nextHourStatus = useMemo(() => {
    ...
    const zoneHourlyTarget = Math.round(selectedZone.hourlySales);
    const theoreticalTargetSoFar = Math.round(zoneHourlyTarget * hoursElapsedInZone);
    ...
    // Calculated missed sales from hours that passed
    // If actualSalesInZone is less than expectedSoFar, we have missed sales.
    const missedSales = Math.max(0, theoreticalTargetSoFar - actualSalesInZone);

    if (hoursRemaining > 0) {
      // Standard hourly target plus the missed sales from the elapsed hours, added fully to the next hour!
      requiredNext = Math.round(zoneHourlyTarget + missedSales);
    } else {
      // If we are on the last hour, the target is simply whatever remains to complete the zone's total target
      requiredNext = Math.max(0, selectedZone.totalSales - actualSalesInZone);
    }
    ...
    if (difference > 2) {
      ...
      status = "behind";
      message = `Είστε πίσω από το στόχο της ζώνης κατά ${Math.abs(difference)} τεμ. Οι πωλήσεις που χάθηκαν προστέθηκαν στον στόχο της αμέσως επόμενης ώρας. Προτείνεται: 1) Combo Specialty Espresso με σνακ, 2) Καμπάνια στα social media, 3) Ήπιο Up-selling!`;
    } else {
      status = "on-track";
    ...
    return {
      requiredNext: Math.round(requiredNext),
      status,
      message,
      theoreticalTargetSoFar: Math.round(theoreticalTargetSoFar)
    };
  ...
  // Load actual dynamic data from Chapter 8 (Unified Financials) based on daily actual sales (πωλήσεις ημέρας)
  const chapter8Metrics = useMemo(() => {
    try {
      const saved = localStorage.getItem("thess_cult_hub_unified_years_financials");
      const activeYear = localStorage.getItem("thess_cult_hub_unified_selected_year") || "2026";
      const activeMonthStr = localStorage.getItem("thess_cult_hub_unified_selected_month");
      const activeMonth = activeMonthStr !== null ? Number(activeMonthStr) : 5; // default to June (5)
      ...
      const parsed = JSON.parse(saved);
      const yearData = parsed[activeYear];
      if (!yearData) {
        return {
          revenue: 0,
          cogs: 0,
      ...
      // Fixed cost for the selected month to keep it connected
      const fixedCost = (yearData.fixedExpenses || []).reduce((sum: number, f: any) => sum + (f.monthlyCost || 0), 0);
      
      // Load actual daily actuals from the database/localStorage
      const savedActuals = localStorage.getItem("thess_cult_hub_unified_daily_actuals");
      let totalActualRevenue = 0;
      let totalActualCogs = 0;
      
      if (savedActuals) {
        const dailyActuals = JSON.parse(savedActuals);
        const daysInMonthNum = new Date(Number(activeYear), activeMonth + 1, 0).getDate();
        const monthStr = String(activeMonth + 1).padStart(2, "0");
        
        for (let d = 1; d <= daysInMonthNum; d++) {
          const dayStr = String(d).padStart(2, "0");
          const dateKey = `${activeYear}-${monthStr}-${dayStr}`;
          ...
          if (log) {
            ...
            // Collect revenues & cogs for products logged on this day
            if (log.products) {
              Object.entries(log.products).forEach(([pId, qty]) => {
                const product = (yearData.products || []).find((p: any) => p.id === pId);
                if (product) {
                  dayRevenue += Number(qty || 0) * (product.price || 0);
                  dayCogs += Number(qty || 0) * (product.cogs || 0);
                }
              });
            }
            
            // Collect revenues & cogs for services logged on this day
            if (log.services) {
              Object.entries(log.services).forEach(([sId, qty]) => {
                const service = (yearData.services || []).find((s: any) => s.id === sId);
                if (service) {
                  dayRevenue += Number(qty || 0) * (service.price || 0);
                  dayCogs += Number(qty || 0) * (service.cogs || 0);
                }
              });
            }
            
            totalActualRevenue += dayRevenue;
            totalActualCogs += dayCogs;
          }
        }
      }
      ...
      return {
        ...
        cogs: totalActualCogs,
        fixedExpenses: fixedCost,
        activeYear,
        activeMonth
      };
    } catch (e) {
      console.warn("Failed to parse Chapter 8 metrics:", e);
      return {
        revenue: 0,
        cogs: 0,
        fixedExpenses: 0,
  ...
  const totalMonthlyRevenueCombined = useMemo(() => {
    if (chapter8Metrics) {
      return chapter8Metrics.revenue;
    }
    return monthlyCoffeeSalesRevenue + monthlyExtraRevenue;
  }, [chapter8Metrics, monthlyCoffeeSalesRevenue, monthlyExtraRevenue]);

  const coffeeCOGS_ToUse = useMemo(() => {
    if (chapter8Metrics) {
      return chapter8Metrics.cogs;
    }
    return monthlyCoffeeCostOfGoodsSold;
  }, [chapter8Metrics, monthlyCoffeeCostOfGoodsSold]);

  const fixedExpenses_ToUse = useMemo(() => {
    if (chapter8Metrics) {
      return chapter8Metrics.fixedExpenses;
    }
    return fixedExpenses.reduce((acc: number, exp: any) => acc + exp.amount, 0);
  }, [chapter8Metrics, fixedExpenses]);

  const totalMonthlyFixedExpenses = fixedExpenses_ToUse;

  const netMonthlyProfitPreTax = totalMonthlyRevenueCombined - coffeeCOGS_ToUse - fixedExpenses_ToUse;

  // Break-even Calculations in daily euros
  const monthlyFixedCostCombined = fixedExpenses_ToUse;
  const contributionMarginPercent = useMemo(() => {
    if (chapter8Metrics && chapter8Metrics.revenue > 0) {
      return (chapter8Metrics.revenue - chapter8Metrics.cogs) / chapter8Metrics.revenue;
    }
    return (averagePrice - averageCost) / averagePrice;
  }, [chapter8Metrics, averagePrice, averageCost]);
  ...
  const applyCloudData = (data: HubDataBundle, isImport: boolean = false) => {
    if (!data) return;
    ...
    // Save unified financial year details to localStorage
    if (data.unifiedYearsFinancials) {
      localStorage.setItem("thess_cult_hub_unified_years_financials", JSON.stringify(data.unifiedYearsFinancials));
    }
    if (data.unifiedDailyActuals) {
      localStorage.setItem("thess_cult_hub_unified_daily_actuals", JSON.stringify(data.unifiedDailyActuals));
    }
    if (data.unifiedManualRevenues) {
      localStorage.setItem("thess_cult_hub_manual_revenues", JSON.stringify(data.unifiedManualRevenues));
    }
  ...
  const buildCurrentBundle = (): HubDataBundle => {
    return {
      ...
      aiCommentary,
      sitePassword,
      unifiedYearsFinancials: (() => {
        try {
          const s = localStorage.getItem("thess_cult_hub_unified_years_financials");
          return s ? JSON.parse(s) : undefined;
        } catch (e) { return undefined; }
      })(),
      unifiedDailyActuals: (() => {
        try {
          const s = localStorage.getItem("thess_cult_hub_unified_daily_actuals");
          return s ? JSON.parse(s) : undefined;
        } catch (e) { return undefined; }
      })(),
      unifiedManualRevenues: (() => {
        try {
          const s = localStorage.getItem("thess_cult_hub_manual_revenues");
          return s ? JSON.parse(s) : undefined;
        } catch (e) { return undefined; }
      })(),
      updatedBy: sessionId
  ...
  const handleExportBackup = () => {
    try {
      ...
      downloadAnchor.download = `thessaloniki_culture_cafe_backup_${hubId || "default"}_${new Date().toISOString().slice(0, 10)}.json`;
      ...
      alert("Αποτυχία εξαγωγής backup.");
    }
  };
    ...
    fileReader.onload = async (event) => {
      try {
        ...
        if (parsed && typeof parsed === "object") {
          ...
          // Apply to state, passing isImport = true to prevent automatic cancellation in auto-save loop
          applyCloudData(parsed, true);

          // Force writing each of these to localStorage immediately in case the browser gets refreshed/closed!
          try {
            if (parsed.customTabs) localStorage.setItem("thess_cult_hub_custom_tabs", JSON.stringify(parsed.customTabs));
            if (parsed.closureData) localStorage.setItem("thess_cult_hub_closure_data", JSON.stringify(parsed.closureData));
            if (parsed.execSummary) localStorage.setItem("thess_cult_hub_exec_summary", JSON.stringify(parsed.execSummary));
            if (parsed.swot) localStorage.setItem("thess_cult_hub_swot", JSON.stringify(parsed.swot));
            if (parsed.pest) localStorage.setItem("thess_cult_hub_pest", JSON.stringify(parsed.pest));
            if (parsed.industry) localStorage.setItem("thess_cult_hub_industry", JSON.stringify(parsed.industry));
            if (parsed.tows) localStorage.setItem("thess_cult_hub_tows", JSON.stringify(parsed.tows));
            if (parsed.stratChoices) localStorage.setItem("thess_cult_hub_strat_choices", JSON.stringify(parsed.stratChoices));
            if (parsed.funcStrats) localStorage.setItem("thess_cult_hub_func_strats", JSON.stringify(parsed.funcStrats));
            if (parsed.timeline) localStorage.setItem("thess_cult_hub_timeline", JSON.stringify(parsed.timeline));
            if (parsed.risks) localStorage.setItem("thess_cult_hub_risks", JSON.stringify(parsed.risks));
            if (parsed.porter) localStorage.setItem("thess_cult_hub_porter", JSON.stringify(parsed.porter));
            if (parsed.customValues) localStorage.setItem("thess_cult_hub_custom_values", JSON.stringify(parsed.customValues));
            if (parsed.customGoals) localStorage.setItem("thess_cult_hub_custom_goals", JSON.stringify(parsed.customGoals));
            if (parsed.customCapabilities) localStorage.setItem("thess_cult_hub_custom_capabilities", JSON.stringify(parsed.customCapabilities));
            if (parsed.customFuncStrats) localStorage.setItem("thess_cult_hub_custom_func_strats", JSON.stringify(parsed.customFuncStrats));
            if (parsed.marketing) localStorage.setItem("thess_cult_hub_marketing", JSON.stringify(parsed.marketing));
            if (parsed.services) localStorage.setItem("thess_cult_hub_services", JSON.stringify(parsed.services));
            if (parsed.chapters) localStorage.setItem("thess_cult_hub_chapters", JSON.stringify(parsed.chapters));
            if (parsed.visualSystem) localStorage.setItem("thess_cult_hub_visual_system", JSON.stringify(parsed.visualSystem));
            if (parsed.morningTicks) localStorage.setItem("thess_cult_hub_morning_ticks", JSON.stringify(parsed.morningTicks));
            if (parsed.busyTicks) localStorage.setItem("thess_cult_hub_busy_ticks", JSON.stringify(parsed.busyTicks));
            if (parsed.closingTicks) localStorage.setItem("thess_cult_hub_closing_ticks", JSON.stringify(parsed.closingTicks));
            if (parsed.weekdayTarget !== undefined) localStorage.setItem("thess_cult_hub_target_weekday", String(parsed.weekdayTarget));
            if (parsed.weekendTarget !== undefined) localStorage.setItem("thess_cult_hub_target_weekend", String(parsed.weekendTarget));
            if (parsed.sundayTarget !== undefined) localStorage.setItem("thess_cult_hub_target_sunday", String(parsed.sundayTarget));
            if (parsed.operatingZones) localStorage.setItem("thess_cult_hub_operating_zones", JSON.stringify(parsed.operatingZones));
            if (parsed.productCategories) localStorage.setItem("thess_cult_hub_product_categories", JSON.stringify(parsed.productCategories));
            if (parsed.stockProducts) localStorage.setItem("thess_cult_hub_stock_products", JSON.stringify(parsed.stockProducts));
            if (parsed.baristaWorkflow) localStorage.setItem("thess_cult_hub_barista_workflow", JSON.stringify(parsed.baristaWorkflow));
            if (parsed.staffEvaluation) localStorage.setItem("thess_cult_hub_staff_evaluation", JSON.stringify(parsed.staffEvaluation));
            if (parsed.financialProducts) localStorage.setItem("thess_cult_hub_fin_products", JSON.stringify(parsed.financialProducts));
            if (parsed.fixedExpenses) localStorage.setItem("thess_cult_hub_fixed_expenses", JSON.stringify(parsed.fixedExpenses));
            if (parsed.extraServicesRevenue) localStorage.setItem("thess_cult_hub_extra_services", JSON.stringify(parsed.extraServicesRevenue));
            if (parsed.questionnaireAnswers) localStorage.setItem("thess_cult_hub_questionnaire_answers", JSON.stringify(parsed.questionnaireAnswers));
            if (parsed.questionnaireSchema) localStorage.setItem("thess_cult_hub_questionnaire_schema", JSON.stringify(parsed.questionnaireSchema));
            if (parsed.financialResults) localStorage.setItem("thess_cult_hub_fin_results", JSON.stringify(parsed.financialResults));
            if (parsed.aiCommentary !== undefined) localStorage.setItem("thess_cult_hub_ai_commentary", parsed.aiCommentary);
            if (parsed.financialTitle !== undefined) localStorage.setItem("thess_cult_hub_fin_title", parsed.financialTitle);
            if (parsed.shopHours !== undefined) localStorage.setItem("thess_cult_hub_hours", parsed.shopHours);
            if (parsed.sitePassword !== undefined) localStorage.setItem("thess_cult_hub_password", parsed.sitePassword);
            if (parsed.unifiedYearsFinancials) localStorage.setItem("thess_cult_hub_unified_years_financials", JSON.stringify(parsed.unifiedYearsFinancials));
            if (parsed.unifiedDailyActuals) localStorage.setItem("thess_cult_hub_unified_daily_actuals", JSON.stringify(parsed.unifiedDailyActuals));
            if (parsed.unifiedManualRevenues) localStorage.setItem("thess_cult_hub_manual_revenues", JSON.stringify(parsed.unifiedManualRevenues));
            
            // Set the local update timestamp to the current time so it won't be overwritten by older cloud data on reload
            localStorage.setItem("thess_cult_hub_last_local_update_at", new Date().toISOString());
          } catch (storageErr) {
            console.warn("Storage write failed during manual backup import:", storageErr);
          }

          // If cloud sync is enabled, write to Firestore immediately!
          if (cloudSyncEnabled) {
            try {
              // Ensure we tell Firestore who updated it and when
              const finalBundle = {
                ...parsed,
              ...
              await saveHubData(hubId, finalBundle);
              
              // Now update the last applied bundle reference to match the newly saved state precisely
              ...
              alert("✅ Το Backup εισήχθη με επιτυχία! Όλα τα δεδομένα ενημερώθηκαν τοπικά και συγχρονίστηκαν αυτόματα στο Firebase Cloud Database!");
              window.location.reload();
            } catch (cloudErr: any) {
              console.error("Cloud push failed during backup import:", cloudErr);
              if (isQuotaError(cloudErr)) {
                setSyncStatus("quota");
                alert("✅ Το Backup φορτώθηκε και αποθηκεύτηκε τοπικά!\n⚠️ Σφάλμα Cloud (Εξάντληση Quota): Η δωρεάν βάση δεδομένων Firebase εξάντλησε το ημερήσιο όριο εγγραφών. Τα δεδομένα σας είναι 100% ασφαλή τοπικά στον υπολογιστή σας!");
                window.location.reload();
              } else {
                setSyncStatus("error");
                alert("✅ Το Backup φορτώθηκε τοπικά!\n⚠️ Αποτυχία αποθήκευσης στο Cloud. Τα δεδομένα σας αποθηκεύτηκαν με ασφάλεια τοπικά στη συσκευή σας.");
                window.location.reload();
              }
            }
          } else {
            setSyncStatus("offline");
            alert("✅ Το Backup εισήχθη με επιτυχία! Τα δεδομένα ενημερώθηκαν τοπικά στη συσκευή σας.");
            window.location.reload();
          }
        } else {
        ...
        alert("❌ Αποτυχία ανάγνωσης του αρχείου backup. Βεβαιωθείτε ότι είναι σωστό JSON αρχείο.");
      }
    };
  ...
  // Sync products, costs, and sales volumes from Financial Elements (financialProducts) into Stock & Orders (stockProducts)
  const handleSyncFromFinancialProducts = () => {
    try {
      const list = [...stockProducts];
      let addedCount = 0;
      let updatedCount = 0;

      financialProducts.forEach((fProd: any) => {
        const fName = (fProd.name || "").trim().toLowerCase();
        if (!fName) return;

        // Try to find a stock item with a matching name
        const matchIdx = list.findIndex((item: any) => (item.name || "").trim().toLowerCase() === fName);

        if (matchIdx !== -1) {
          // Update existing stock product parameters
          const updatedItem = { ...list[matchIdx] };
          updatedItem.costPerUnit = fProd.cost !== undefined ? fProd.cost : updatedItem.costPerUnit;
          updatedItem.dailySales = fProd.dailySales !== undefined ? fProd.dailySales : updatedItem.dailySales;
          updatedItem.category = fProd.category || updatedItem.category;
          list[matchIdx] = updatedItem;
          updatedCount++;
        } else {
          // Add new stock product
          // Guessing metric unit based on name or category
          let unit = "τεμάχιο";
          if (
            fName.includes("espresso") || 
            fName.includes("cappuccino") || 
            fName.includes("coffee") || 
            fName.includes("filter") || 
            fName.includes("καφέ")
          ) {
            unit = "kg";
          } else if (fName.includes("muffin") || fName.includes("cookie") || fName.includes("σνακ")) {
            unit = "τεμάχιο";
          } else if (fName.includes("beer") || fName.includes("μπύρ") || fName.includes("cocktail") || fName.includes("ποτό")) {
            unit = "τεμάχιο";
          }

          const newStockItem = {
            id: `stock-sync-${Date.now()}-${Math.random().toString(36).substr(2, 4)}`,
            name: fProd.name,
            category: fProd.category || "coffee",
            unit: unit,
            currentStock: 0,
            dailySales: fProd.dailySales !== undefined ? fProd.dailySales : 10,
            daysToCover: 3,
            costPerUnit: fProd.cost !== undefined ? fProd.cost : 0.50
          };
          list.push(newStockItem);
          addedCount++;
        }
      });

      setStockProducts(list);
      alert(`✅ Ο συγχρονισμός ολοκληρώθηκε!\n• Ενημερώθηκαν: ${updatedCount} προϊόντα\n• Προστέθηκαν νέα: ${addedCount} προϊόντα\n\nΤα κόστη μονάδας και οι ημερήσιες πωλήσεις τώρα ταυτίζονται με τα Οικονομικά Στοιχεία!`);
    } catch (err) {
      console.error("Failed to sync financial products to stock:", err);
      alert("❌ Αποτυχία συγχρονισμού δεδομένων.");
    }
  };
  ...
  useEffect(() => {
    ...
    fetchHubData(hubId)
      .then((data) => {
        if (data) {
          const localLastUpdatedStr = localStorage.getItem("thess_cult_hub_last_local_update_at");
          const cloudUpdatedAtStr = data.updatedAt;
          
          let useCloud = true;
          if (localLastUpdatedStr) {
            if (cloudUpdatedAtStr) {
              const localTime = new Date(localLastUpdatedStr).getTime();
              const cloudTime = new Date(cloudUpdatedAtStr).getTime();
              // Cloud must be strictly newer than local by at least 2 seconds to overwrite local state
              if (cloudTime > localTime + 2000) {
                useCloud = true;
              } else {
                useCloud = false;
                console.info(`Preserving newer local/imported data. Local: ${localLastUpdatedStr}, Cloud: ${cloudUpdatedAtStr}`);
              }
            } else {
              useCloud = false;
              console.info("Preserving local data because cloud data is missing an updatedAt timestamp on mount.");
            }
          }
          ...
          if (useCloud) {
            applyCloudData(data);
            if (data.updatedAt) {
              localStorage.setItem("thess_cult_hub_last_local_update_at", data.updatedAt);
            }
            ...
            // Since we are preserving local data, we compile and set lastAppliedBundleRef so
            // that we don't trigger immediate redundant auto-saves of unedited data.
            const localBundle = buildCurrentBundle();
            lastAppliedBundleRef.current = JSON.stringify({
              ...localBundle,
              updatedAt: undefined,
              updatedBy: undefined,
            ...
            if (localLastUpdatedStr) {
              try {
                const localTimeDate = new Date(localLastUpdatedStr);
                setLastSyncedTime(localTimeDate.toLocaleTimeString("el-GR"));
              } catch (_) {
                setLastSyncedTime(new Date().toLocaleTimeString("el-GR"));
              }
            } else {
        ...
        if (localLastUpdatedStr) {
          if (cloudUpdatedAtStr) {
            ...
            if (cloudTime > localTime + 2000) {
              ...
              useCloud = false;
              console.info("Preserving newer local data during real-time snapshot update.");
            }
          } else {
            useCloud = false;
            console.info("Preserving local data because cloud data is missing an updatedAt timestamp during real-time update.");
          }
        }
  ...
  useEffect(() => {
    ...
    // A real local modification has occurred! Update local change timestamp:
    localStorage.setItem("thess_cult_hub_last_local_update_at", new Date().toISOString());
      ...
      <header className="border-b border-zinc-805 bg-[#0f1112] sticky top-0 z-40 px-4 py-3 sm:px-6 md:px-8">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {/* Offline-Safe Local Backup & Transfer Widget */}
            <div className="flex items-center gap-2 bg-zinc-950 px-2.5 py-1.5 rounded-lg border border-zinc-900 text-xs shadow-inner">
              <span className="text-[10px] font-mono font-bold text-zinc-400 uppercase tracking-tight hidden lg:inline">
                Τοπικο αρχειο (PC ⟷ Web):
              </span>
              <button
                onClick={handleExportBackup}
                title="Εξαγωγή όλων των δεδομένων σε αρχείο JSON (Λήψη Backup)"
                className="flex items-center gap-1 px-2 py-1 rounded bg-zinc-900 border border-zinc-800 text-[10px] font-mono text-zinc-300 hover:text-amber-400 hover:border-amber-500/40 hover:bg-zinc-850 transition cursor-pointer"
              >
                <Download className="w-3.5 h-3.5 text-amber-500" />
                <span>Εξαγωγη (Download)</span>
              </button>

              <label
                title="Εισαγωγή δεδομένων από αρχείο JSON Backup"
                className="flex items-center gap-1 px-2 py-1 rounded bg-zinc-900 border border-zinc-800 text-[10px] font-mono text-zinc-300 hover:text-emerald-400 hover:border-emerald-500/40 hover:bg-zinc-850 transition cursor-pointer"
              >
                <Upload className="w-3.5 h-3.5 text-emerald-500" />
                <span>Εισαγωγη (Upload)</span>
                <input
                  type="file"
                  accept=".json"
            ...
            <div className="hidden lg:flex items-center gap-3 text-xs font-mono bg-zinc-900/40 p-1.5 px-3 rounded-lg border border-zinc-800/80">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-zinc-400 col-span-1">
                  ΤΖΙΡΟΣ ({"ΙΑΝ_ΦΕΒ_ΜΑΡ_ΑΠΡ_ΜΑΪ_ΙΟΥΝ_ΙΟΥΛ_ΑΥΓ_ΣΕΠ_ΟΚΤ_ΝΟΕ_ΔΕΚ".split("_")[chapter8Metrics?.activeMonth ?? 5]} {chapter8Metrics?.activeYear ?? "2026"}):
                </span>
                  ...
                  <table className="w-full text-xs font-mono text-[#f4f4f5] text-left border-collapse">
                    <thead>
                      <tr className="border-b border-zinc-800 bg-zinc-950/40 text-[10px] font-bold uppercase tracking-wider text-zinc-400">
                        ...
                        <th className="py-2.5 px-3 text-right border-r border-zinc-900 font-bold">Ημερ. Πωλήσεις (τεμ)</th>
                          ...
                          <td className="py-2 px-3 text-right border-r border-[#1e2022]">
                            {editMode ? (
                              <input
                                type="number"
                                step="1"
                                value={p.dailySales !== undefined ? p.dailySales : 10}
                                onChange={(e) => {
                                  const list = [...financialProducts];
                                  list[index] = { ...list[index], dailySales: Number(e.target.value) };
                                  setFinancialProducts(list);
                                }}
                                className="bg-[#1e2022] border border-zinc-800 text-zinc-150 p-1 rounded font-mono text-xs w-16 text-right focus:ring-1 focus:ring-amber-500"
                              />
                            ) : (
                              <span>{p.dailySales !== undefined ? p.dailySales : 10}</span>
                            )}
                          </td>
            ...
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="bg-[#141618] border border-zinc-800 rounded-xl p-6 space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  {/* Dynamic Financials Sync Tool */}
                  <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-900/80 flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="space-y-1 text-left">
                      <span className="text-xs font-bold text-amber-500 uppercase font-mono flex items-center gap-1.5">
                        <RefreshCw className="w-3.5 h-3.5" />
                        <span>🔗 Διασύνδεση με Οικονομικά Στοιχεία (Κεφ. 5.1 & 5.2)</span>
                      </span>
                      <p className="text-[11px] text-zinc-400 leading-relaxed max-w-2xl">
                        Συνδέστε τα δύο κεφάλαια! Με ένα κλικ, η εφαρμογή μεταφέρει αυτόματα τα προϊόντα, τις κατηγορίες, τα κόστη παραγωγής (COGS) και τις ημερήσιες πωλήσεις από τα Οικονομικά Στοιχεία απευθείας στον Υπολογισμό Παραγγελιών.
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={handleSyncFromFinancialProducts}
                      className="flex items-center gap-1.5 px-4 py-2 bg-zinc-900 border border-zinc-800 hover:border-amber-500 hover:text-white text-xs font-bold text-amber-400 font-mono rounded-lg cursor-pointer transition active:scale-95 shadow-lg select-none shrink-0"
                      title="Λήψη ονομάτων, κατηγοριών, κόστους παραγωγής (COGS) και ημερήσιων πωλήσεων από το Κεφάλαιο 5.1/5.2"
                    >
                      <RefreshCw className="w-3.5 h-3.5 animate-spin" style={{ animationDuration: '6s' }} />
                      <span>ΣΥΓΧΡΟΝΙΣΜΟΣ ΔΕΔΟΜΕΝΩΝ</span>
                    </button>
                  </div>
