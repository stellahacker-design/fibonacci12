...
export default function UnifiedFinancials() {
  ...
  // Calculations scaled for active period (Month / Q / 6M / Year)
  const financialMetrics = useMemo(() => {
    const { products, services, fixedExpenses } = activeYearData;

    // Revenue breakdown
    const productItemsData = products.map(p => {
      const revenue = p.price * p.monthlyVolume * multiplier;
      const cogs = p.cogs * p.monthlyVolume * multiplier;
      ...
      return {
        ...
        cogs,
        profit,
        volume: p.monthlyVolume * multiplier
      };
    });

    const serviceItemsData = services.map(s => {
      const revenue = s.price * s.monthlyVolume * multiplier;
      const cogs = s.cogs * s.monthlyVolume * multiplier;
      const profit = revenue - cogs;
      return {
        id: s.id,
        name: s.name,
        ...
        cogs,
        profit,
        volume: s.monthlyVolume * multiplier
      };
    });
    ...
    const totalRevenue = productRevSum + serviceRevSum;
    ...
    const totalCOGS = productCogsSum + serviceCogsSum;
  ...
  const yearsAggregates = useMemo(() => {
    ...
    Object.keys(yearsData).forEach(yr => {
      ...
      if (!entry) return;
      const pRev = entry.products.reduce((acc, p) => acc + (p.price * p.monthlyVolume * 12), 0);
      const sRev = entry.services.reduce((acc, s) => acc + (s.price * s.monthlyVolume * 12), 0);
      const pCogs = entry.products.reduce((acc, p) => acc + (p.cogs * p.monthlyVolume * 12), 0);
      const sCogs = entry.services.reduce((acc, s) => acc + (s.cogs * s.monthlyVolume * 12), 0);
      const fixed = entry.fixedExpenses.reduce((acc, f) => acc + (f.monthlyCost * 12), 0);

      const yrRevenue = pRev + sRev;
      const yrProfit = yrRevenue - (pCogs + sCogs + fixed);
      result[yr] = { totalRevenue: yrRevenue, netProfit: yrProfit };
    });
    return result;
  }, [yearsData]);
            ...
            {activeSummaryTab === "plan" ? (
              <div className="space-y-4">
                <span className="text-xs font-mono text-emerald-400 font-bold uppercase tracking-wider block pb-2 border-b border-zinc-850">
                  🎯 ΣΥΝΟΨΗ {periodSpec.label.toUpperCase()}
                </span>
                ...
                <div className={`p-4 rounded-xl border flex flex-col justify-center mt-4 ${
                  ...
                  <p className="text-[10px] mt-1 text-zinc-400 leading-normal">
                    Για το {periodSpec.text.toLowerCase()} του έτους {selectedYear} με βάση τις τρέχουσες επεξεργάσιμες παραμέτρους.
                  </p>
                </div>
              </div>
            ) : (
            ...
            <div className="p-5 border-b border-[#202327] flex items-center justify-between bg-zinc-950/40 no-print">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-zinc-500">Πραγματικά αποτελέσματα καταγραφής συγκριτικά με τους στόχους</p>
                </div>
              </div>
              <button 
            ...
            <div className="p-6 overflow-y-auto space-y-6 text-zinc-350 p-bg-white p-text-black print-container flex-1">
              ...
              <div className="space-y-2 p-text-black">
                <h4 className="text-xs font-mono font-extrabold uppercase text-[#10b981] p-text-black tracking-wider border-b p-border-gray pb-1">
                  📦 ΠΡΑΓΜΑΤΙΚΕΣ ΠΩΛΗΣΕΙΣ ΠΡΟΪΟΝΤΩΝ & ΣΤΟΧΟΙ
                </h4>
                  ...
                  <table className="w-full text-left font-sans text-xs border-collapse">
                    <thead>
                      <tr className="border-b p-border-gray text-zinc-400 p-text-black">
                        ...
                        <th className="py-2 text-right font-bold font-sans">Plan Στόχος (Tεμ)</th>
                        <th className="py-2 text-right font-bold font-sans">Ποσοστό %</th>
                    ...
                    <tbody>
                      {monthlyItemizedActuals.productsDetails.map((p) => (
                        <tr key={p.id} className="border-b p-border-gray text-zinc-300 p-text-black hover:bg-zinc-900/10">
                          ...
                          <td className="py-2.5 text-right font-mono text-zinc-500 p-text-black">{p.monthlyVolume} τεμ</td>
                          <td className="py-2.5 text-right font-mono font-bold">
                            <span className={p.completionRate >= 100 ? "text-emerald-400 p-text-black" : p.completionRate >= 50 ? "text-amber-400 p-text-black" : "text-zinc-500 p-text-black"}>
              ...
              <div className="space-y-2 p-text-black">
                <h4 className="text-xs font-mono font-extrabold uppercase text-[#10b981] p-text-black tracking-wider border-b p-border-gray pb-1">
                  💼 ΠΡΑΓΜΑΤΙΚΕΣ ΠΩΛΗΣΕΙΣ ΥΠΗΡΕΣΙΩΝ & ΣΤΟΧΟΙ
                </h4>
                <div className="overflow-x-auto">
                  <table className="w-full text-left font-sans text-xs border-collapse">
                    <thead>
                    ...
                    <tbody>
                      {monthlyItemizedActuals.servicesDetails.map((s) => (
                        <tr key={s.id} className="border-b p-border-gray text-zinc-300 p-text-black hover:bg-zinc-900/10">
                          ...
                          <td className="py-2.5 text-right font-mono text-zinc-500 p-text-black">{s.monthlyVolume} τεμ</td>
                          <td className="py-2.5 text-right font-mono font-bold">
                            <span className={s.completionRate >= 100 ? "text-emerald-400 p-text-black" : s.completionRate >= 50 ? "text-amber-400 p-text-black" : "text-zinc-500 p-text-black"}>
