...
export default function App() {
  ...
  // Load actual dynamic data from Chapter 8 (Unified Financials) if it exists
  const chapter8Metrics = useMemo(() => {
    try {
      ...
      const prodRevenue = (yearData.products || []).reduce((sum: number, p: any) => sum + ((p.price || 0) * (p.monthlyVolume || 0)), 0);
      const servRevenue = (yearData.services || []).reduce((sum: number, s: any) => sum + ((s.price || 0) * (s.monthlyVolume || 0)), 0);
      
      const prodCOGS = (yearData.products || []).reduce((sum: number, p: any) => sum + ((p.cogs || 0) * (p.monthlyVolume || 0)), 0);
      const servCOGS = (yearData.services || []).reduce((sum: number, s: any) => sum + ((s.cogs || 0) * (s.monthlyVolume || 0)), 0);
      
      const fixedCost = (yearData.fixedExpenses || []).reduce((sum: number, f: any) => sum + (f.monthlyCost || 0), 0);
      
      const totalRev = prodRevenue + servRevenue;
      if (totalRev === 0) return null; // Fallback if no volumes entered yet
      
      return {
        revenue: totalRev,
        cogs: prodCOGS + servCOGS,
