...
import { saveHubData, fetchHubData, subscribeToHubData, HubDataBundle, DEFAULT_HUB_ID } from "./firebase";
...
export default function App() {
  ...
  const [syncStatus, setSyncStatus] = useState<"idle" | "loading" | "synced" | "saving" | "error" | "offline">("idle");
  ...
  useEffect(() => {
    ...
    // 2. Setup Real-time Listener (onSnapshot) to receive other people's edits while in Preview/View mode
    const unsubscribe = subscribeToHubData(hubId, (data) => {
      // Avoid overwriting active inputs when the user is editing locally (to prevent cursor stuttering)
      if (!editMode && data) {
        applyCloudData(data);
        setSyncStatus("synced");
        if (data.updatedAt) {
  ...
  }, [hubId, cloudSyncEnabled, editMode]);

  // 3. Debounced Auto-Save to Cloud when changes occur in Edit Mode
  useEffect(() => {
    if (!cloudSyncEnabled || !editMode) return;
    ...
    }, 2500); // 2.5 second idle delay to prevent spamming firestore requests
...
`;
    ...
    try {
      ...
      if (data.success) {
        ...
        updated[selectedClosurePeriod].commentary = data.commentary;
      ...
      if (!activeKey) {
        ...
        const updated = { ...closureData };
        updated[selectedClosurePeriod].commentary = errorMsg;
        setClosureData(updated);
      ...
      try {
        ...
        if (directJson.candidates && directJson.candidates[0]?.content?.parts?.[0]?.text) {
          const text = directJson.candidates[0].content.parts[0].text;
          const updated = { ...closureData };
          updated[selectedClosurePeriod].commentary = text;
          setClosureData(updated);
        } else {
          const apiMsg = directJson.error?.message || "Μη έγκυρη απόκριση από το Google API.";
          const updated = { ...closureData };
          updated[selectedClosurePeriod].commentary = `⚠️ Το Google API επέστρεψε σφάλμα: ${apiMsg}`;
          setClosureData(updated);
        }
      } catch (fallbackErr: any) {
        console.error("Direct fallback closure analysis failed too:", fallbackErr);
        const updated = { ...closureData };
        updated[selectedClosurePeriod].commentary = `⚠️ Σφάλμα άμεσης σύνδεσης με Gemini API: ${fallbackErr.message || fallbackErr}`;
        setClosureData(updated);
      }
    } finally {
      setAiClosureLoading(false);
              ...
              <div className="flex items-center gap-1.5 pr-2.5 border-r border-zinc-850">
                <span className="relative flex h-2.5 w-2.5">
                  <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${
                    ...
                    syncStatus === "loading" ? "bg-blue-400" : "bg-rose-400"
                  }`}></span>
                  <span className={`relative inline-flex rounded-full h-2.5 w-2.5 ${
                    syncStatus === "synced" ? "bg-emerald-500" :
                    syncStatus === "saving" ? "bg-amber-500" :
                    syncStatus === "loading" ? "bg-blue-500" : "bg-rose-500"
                  }`}></span>
                </span>
                <span className="text-[10px] font-mono font-bold text-zinc-300 uppercase tracking-tight hidden lg:inline">
                  {syncStatus === "synced" ? "Cloud Synced" :
                   ...
                   syncStatus === "offline" ? "Cloud Disabled" : "Sync Error"}
                </span>
              </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-[#141618] border border-zinc-800 rounded-xl p-5 space-y-4">
                      <div className="flex justify-between items-center pb-2 border-b border-zinc-800">
                        ...
                        <span className="text-[10px] font-mono text-amber-500">Πίνακας Δεδομένων</span>
                      </div>
                                  ...
                                  <td className="py-2.5 font-mono text-zinc-300">
                                    {editMode ? (
                                      <input
                                        ...
                                        onChange={(e) => {
                                          const updated = { ...closureData };
                                          updated[selectedClosurePeriod].chartData[idx].name = e.target.value;
                                          // Recalculate totals
                                    ...
                                    {editMode ? (
                                      <input
                                        ...
                                        onChange={(e) => {
                                          ...
                                          const updated = { ...closureData };
                                          updated[selectedClosurePeriod].chartData[idx].sales = val;
                                          updated[selectedClosurePeriod].chartData[idx].profit = val - item.cogs - item.expenses;
                                          const totals = updated[selectedClosurePeriod].chartData.reduce(
                                            (acc: any, cur: any) => {
                                              acc.sales += cur.sales;
                                              acc.cogs += cur.cogs;
                                    ...
                                    {editMode ? (
                                      <input
                                        ...
                                        onChange={(e) => {
                                          const val = Math.max(0, Number(e.target.value));
                                          const updated = { ...closureData };
                                          updated[selectedClosurePeriod].chartData[idx].cogs = val;
                                          updated[selectedClosurePeriod].chartData[idx].profit = item.sales - val - item.expenses;
                                          const totals = updated[selectedClosurePeriod].chartData.reduce(
                                            (acc: any, cur: any) => {
                                              acc.sales += cur.sales;
                                              acc.cogs += cur.cogs;
                                    ...
                                    {editMode ? (
                                      <input
                                        ...
                                        onChange={(e) => {
                                          const val = Math.max(0, Number(e.target.value));
                                          const updated = { ...closureData };
                                          updated[selectedClosurePeriod].chartData[idx].expenses = val;
                                          updated[selectedClosurePeriod].chartData[idx].profit = item.sales - item.cogs - val;
                                          const totals = updated[selectedClosurePeriod].chartData.reduce(
                                            (acc: any, cur: any) => {
                                              acc.sales += cur.sales;
                                              acc.cogs += cur.cogs;
                      ...
                      {editMode && (
                        <div className="flex gap-2 justify-end">
                          <button
                            ...
                            onClick={() => {
                              const updated = { ...closureData };
                              const length = updated[selectedClosurePeriod].chartData.length;
                              updated[selectedClosurePeriod].chartData.push({
                          ...
                          <button
                            ...
                            onClick={() => {
                              const updated = { ...closureData };
                              if (updated[selectedClosurePeriod].chartData.length > 1) {
                                updated[selectedClosurePeriod].chartData.pop();
