...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Detect if layout/design settings (visualSystem) changed specifically
    const visualChanged = JSON.stringify(prevVisualSystemRef.current) !== JSON.stringify(visualSystem);

    // Save only if editMode is active OR if the visualSystem layout presets/options have been adjusted
    if (!editMode && !visualChanged) {
      return;
    }
    ...
    const delayDebounce = setTimeout(() => {
      ...
      // Persist the updated design setup inside our reference
