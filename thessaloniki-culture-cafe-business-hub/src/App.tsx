...
import { saveHubData, fetchHubData, subscribeToHubData, HubDataBundle, DEFAULT_HUB_ID, isOfflineError } from "./firebase";
...
export default function App() {
  ...
  const [visualSystem, setVisualSystem] = useState(() => {
    ...
    if (!state || !state.mode || state.mode === "stark-beige-punk" || state.mode === "underground-black" || state.mode === "industrial-concrete" || state.mode === "kraft-cardboard") {
      state = {
    ...
    // Ensure corners is rounded for the pastel mood
    if (state && (state.mode === "minimal-pastel-mural" || !state.corners)) {
  ...
  // 3. Debounced Auto-Save to Cloud when changes occur in Edit Mode (high responsiveness)
  useEffect(() => {
